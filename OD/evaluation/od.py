...
def get_args():
    ...
    parser.add_argument('--v2', action='store_true', default=False,
                        help='Is it SeaDronesSee Object Detection v2?')
    parser.add_argument('--skip_sub_1', action='store_true', default=False,
                        help='Skip SeaDronesSee Object Detection v2 subtrack 1')
...
def v1_od():
    ...
    ground_truth = os.path.join(
        ROOT_DIR, 'annotations', 'instances_' + 'val_iscrowd' + '.json')
...
def v2_od_subtrack1():
    ...
    ground_truth = os.path.join(
        ROOT_DIR, 'annotations', 'od_v2_annotations', 'instances_' + 'val_iscrowd' + '.json')
    return evaluate_coco(opt.file_name, categories, ground_truth)
...
def v2_od_subtrack2(filename):
    # non-water
    categories = [0]
    ground_truth = os.path.join(ROOT_DIR, 'annotations', 'od_v2_annotations',
                                'instances_' + 'val_iscrowd' + '_non_water' + '.json')
