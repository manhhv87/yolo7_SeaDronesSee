...
one color for each category (excluding category nr 7)
    ...
    7: ignored
'''
colors = {0: (139, 0, 0), 1: (0, 255, 0), 2: (0, 0, 255), 3: (
    255, 255, 51), 4: (0, 255, 255), 5: (255, 102, 255), 6: (51, 153, 255)}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pictures', type=str, default=None,
        ...
        for image in data:
            ...
            if os.path.exists(outputPath + "/" + str(image['image_id']) + ".jpg"):
                img = cv2.imread(outputPath + "/" +
                                 str(image['image_id']) + ".jpg")
            else:
                img = cv2.imread(options.pictures + "/" +
                                 str(image['image_id']) + ".jpg")
            start = time.time()
            ...
            cv2.rectangle(img, (topLeftX, topLeftY), (topLeftX + width,
                          topLeftY + height), colors[image['category_id']], 3)
            cv2.imwrite(outputPath + "/" +
                        str(image['image_id']) + ".jpg", img)
            print('Picture generation took {0} second! Finished: {1}%'.format(
                time.time() - start, round((currentImage / numberOfImages) * 100, 2)))
