...
def main():
    ...
    parser.add_argument('--image_size', default='1080x1920',
                        type=str, help='[height]x[width]')
    ...
    log_print(
        f'Cuda available: {torch.cuda.is_available()}', model_dir, args.create_log)
    ...
    resize = (int(args.image_size.split('x')[0]), int(
        args.image_size.split('x')[1]))
    log_print(f'Images resized to: {args.image_size}',
    ...
    log_print(f'Using {args.backbone} as CNN backbone',
              model_dir, args.create_log)
    if args.backbone == 'resnet18':
        modules = list(
            resnet18(weights=ResNet18_Weights.DEFAULT).children())[:-2]
        backbone = nn.Sequential(*modules)
        backbone.out_channels = 512
    elif args.backbone == 'resnet50':
        modules = list(
            resnet50(weights=ResNet50_Weights.DEFAULT).children())[:-2]
        backbone = nn.Sequential(*modules)
        backbone.out_channels = 2048
    elif args.backbone == 'resnet101':
        modules = list(
            resnet101(weights=ResNet101_Weights.DEFAULT).children())[:-2]
        backbone = nn.Sequential(*modules)
        backbone.out_channels = 2048
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for images, targets in data_loader_test:
                ...
                loss_dict = model(images, targets)
                average_loss = sum(
                    loss for loss in loss_dict.values()) / args.batch_size
