    ...
    for key in data['annotations']:
        for image in data['images']:
            if (key['image_id'] == image['id']):
                h_img = image['height']
                w_img = image['width']
                yoloFormat = convertToYolo(
                    key['bbox'][0], key['bbox'][1], key['bbox'][2], key['bbox'][3], w_img, h_img)
                f = open(outputPath + "\\" +
                         str(key['image_id']) + ".txt", "a")
                f.write(str(key['category_id']) + " " + str(yoloFormat[0]) + " " + str(
                    yoloFormat[1]) + " " + str(yoloFormat[2]) + " " + str(yoloFormat[3]) + "\n")
                f.close()
                break
    CocoFormat.close()
...
parser.add_argument('--train', action = 'store_true',
                    help = 'convert the train annotation')
parser.add_argument('--test', action = 'store_true',
                    help = 'convert the test annotation')
...
if (options.val == None and options.train == None and options.test == None):
    ...
    exit()
if (options.val):
    computeInput(options.annotations + "\instances_val.json",
                 options.labels + "\\val\labels")
if (options.train):
    computeInput(options.annotations + "\instances_train.json",
                 options.labels + "\\train\labels")
if (options.test):
    computeInput(options.annotations + "\instances_test.json",
                 options.labels + "\\test\labels")
print("Done")
