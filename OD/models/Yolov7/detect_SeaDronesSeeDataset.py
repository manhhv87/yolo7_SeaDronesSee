            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        # convert bboxes from yolo format to coco format
                        w_img = im0.shape[1]
                        h_img = im0.shape[0]
                        w_coco = xywh[2] * w_img
                        h_coco = xywh[3] * h_img
                        x_coco = xywh[0] * w_img - w_coco/2
                        y_coco = xywh[1] * h_img - h_coco/2
                        xywh = [x_coco, y_coco, w_coco, h_coco]
                        # label format
                        line = (
                            cls, *xywh, conf) if opt.save_conf else (cls, *xywh)
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(('%g ' * len(line)).rstrip() % line + '\n')
                        output.append({"image_id": int(p.stem), "category_id": int(
                            f'{int(cls)}'), "bbox": xywh, "score": float(conf)})
