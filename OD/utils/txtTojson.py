...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default=None,
                        help='Path to the output json txt file')
    options = parser.parse_args()
    if (options.txt == None):
        raise ValueError("Please enter a valid path to the txt file")
    if options.annotation == None:
        raise ValueError(
            "Please enter a valid path to the original annotation json file")
    ...
    for line in lines:
        ...
        t_bbox = [x1, y1, w, h]
        t_area = w * h
        annot = {'id': t_id, 'image_id': t_image_id,
                 'bbox': t_bbox, 'area': t_area, 'category_id': t_ctg}
