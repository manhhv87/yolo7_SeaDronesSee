...
colors = {1: (0, 255, 0), 2: (0, 0, 255), 3: (255, 255, 51), 4: (
    0, 255, 255), 5: (255, 102, 255), 6: (51, 153, 255)}
...
if __name__ == '__main__':
    ...
    if options.annotation == None:
        raise ValueError(
            "Please enter a valid path to the annotation json file")
    if options.images == None:
        raise ValueError(
            "Please enter a valid path to folder where the images are located")
    ...
    with open(options.annotation) as f:
        ...
        fps = 30
        count = 0
        videoName = options.output + "\\video_" + \
            str(fps) + "fps_" + str(count) + ".mp4"
        ...
        for image in data['images']:
            ...
            saltitude = 'altitude= ' + \
                str("%.2f" % (image['meta']['altitude'])) + ' m'
            speed = round(image['meta']['speed'])
            sspeed = 'speed= ' + str("%.2f" %
                                     (image['meta']['speed'])) + ' km/h'
            sangle = 'angle= ' + \
                str("%.2f" % (image['meta']['gimbal_pitch'])) + ' degrees'
                ...
                if annotation['image_id'] == image['id']:
                    ...
                    for category in data['categories']:
                        if category['id'] == annotation['category_id']:
                            label = category['name']
                    cv2.rectangle(img, (topLeftX, topLeftY), (topLeftX + width,
                                  topLeftY + height), colors[annotation['category_id']], 3)
                    ps.putBText(img, text=saltitude, text_offset_x=300, text_offset_y=10, vspace=10, hspace=10,
                                font_scale=3.0, background_RGB=(228, 225, 222), text_RGB=(0, 0, 0), thickness=2, alpha=0.5)
                    ps.putBText(img, text=sspeed, text_offset_x=round((img_width/2)-500), text_offset_y=10, vspace=10,
                                hspace=10, font_scale=3.0, background_RGB=(228, 225, 222), text_RGB=(0, 0, 0), thickness=2, alpha=0.5)
                    ps.putBText(img, text=sangle, text_offset_x=img_width-1200, text_offset_y=10, vspace=10, hspace=10,
                                font_scale=3.0, background_RGB=(228, 225, 222), text_RGB=(0, 0, 0), thickness=2, alpha=0.5)
                    # draw lineal
                    cv2.line(img, (15, 0), (15, 2160), (0, 0, 0), thickness=2)
                    cv2.line(img, (215, 0), (215, 2160),
                             (0, 0, 0), thickness=2)
                    d = 0
                    for i in range(15, -1, -1):
                        v_lines_distance = 200
                        cv2.line(img, (15, 150+d), (215, 150+d),
                                 (0, 0, 0), thickness=2)
                        cv2.putText(img, str(i*10)+" m", (240, 150+d+10),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                        d = d + 120

                    drone_y_position = 2160 - 210 - \
                        round(drone_35_65.shape[0]/2) - round(altitude*120/10)

                    if angle < 35:
                        img = cvzone.overlayPNG(
                            img, drone_35, [40, drone_y_position])
                    if angle >= 35 and angle < 65:
                        img = cvzone.overlayPNG(
                            img, drone_35_65, [40, drone_y_position])
                    if angle >= 65:
                        img = cvzone.overlayPNG(
                            img, drone_65, [40, drone_y_position])
            out.write(img)
            print('Finished: {0}%'.format(
                round((currentImage / numberOfImages) * 100, 2)))
        # save video
        out.release()
        scriptRunTime = time.time() - scriptTime
        print('\nScript run time in seconds: {0}'.format(scriptRunTime))
        print("mean time per image: {0}".format(
            scriptRunTime / numberOfImages))
